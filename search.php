...
<table width="400" border="0" cellspacing="0" cellpadding="0">
 <tr>
  <td width="1">Search
  <td width="99%"><input type="text" name="search" size="20" value="<?php echo $search; ?>">
 <tr>
  <td>In category
  <td><select size="1" name="category"><option value="">All</option><?php echo category_get_tree('&nbsp;', '<option value="{id}"[php]search_this_cat({id})[/php]>{prefix}{name}</option>'); ?></select>
 <tr>
  <td><nobr>Date (year/month/date)</nobr>&nbsp;
  <td><?php echo makeDropDown($syear, 'year', $year); ?>/<?php echo makeDropDown($smonth, 'month', $month); ?>/<?php echo makeDropDown($sday, 'day', $day); ?>
 <tr>
...
function formattext($whatfind, $text){
	...
	if ($pos !== false){ //если найдена подстрока
	    if ($pos < $otstup){ //если встречается раньше чем первые N символов
	        ...
	        $result = preg_replace("/$whatfind/i", '<span class="hilite">'.$whatfind.'</span>', $result);
	    } else {
	        $start = $pos-$otstup;
