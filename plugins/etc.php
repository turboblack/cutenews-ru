...
static $uniqid;
	...
	if (!$cache = cute_cache($type.'-'.str_replace(array('/', '?', '&', '='), '-', chicken_dick($_SERVER['REQUEST_URI'])), $uniqid++)){
		...
		if (preg_match('/([0-9]{2}):([0-9]{2}):([0-9]{2})/i', $time)){
			...
			$time = explode(':', $time);
			$time = mktime($time[0], $time[1], $time[2], (preg_match('/[a-z]/i', $month) ? $m_n[$month] : $month), $day, $year);
		}
		...
		if (!$id and !$title and !$time){
			...
			if ($month){
				...
				$result[] = (preg_match('/[a-z]/i', $month) ? str_replace($f_name, $replace, $month) : str_replace($f_num, $replace, $month));
			}
