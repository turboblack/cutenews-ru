...
global $id, $mod;
    ...
    $category = category_get_tree('&nbsp;', '<label for="cat{id}"><input type="checkbox" [php]multicats({id})[/php] name="cat[{id}]" id="cat{id}")">&nbsp;{name}</label><br />');
	if ($category){
	    ...
	    foreach ($urls as $url_k => $url_v){
	        foreach ($url_v as $k => $v){
	            ...
	            for ($i = 0; $i < count($query); $i++){
	                if ($query[$i][0]){
                            $clear = preg_replace('/(.*?)=\$([0-9]+)/i', '', str_replace('$'.$i, $query[$i][0], str_replace('?', '', htaccess_rules_format($v))));
	                    if ($clear){
	                        $str[] = $clear;
	                    }
	                }
...
<form action="<?php echo $PHP_SELF; ?>?mod=options&amp;action=rufus" method="post">
<h3>.htaccess:</h3>
<textarea name="uhtaccess" rows="5" cols="20"<?php echo ($config_rufus != 'yes' ? ' disabled>'.$echo['rufus'] : '>'.$uhtaccess->settings); ?></textarea>
<h3>urls.ini:</h3>
<textarea name="urls_content" rows="5" cols="20"><?php echo file_read(rootpath.'/data/urls.ini'); ?></textarea>
<br /><br />
<input type="submit" name="urls" value="  <?php echo $echo['save']; ?>  "> <input type="submit" name="htaccess" value="   <?php echo ($config_rufus != 'yes' ? $echo['rufus'].'   " disabled' : $echo['make'].'   "'); ?>>
</form>
