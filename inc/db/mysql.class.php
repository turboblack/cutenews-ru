...
class MySQL {
	...
	function table_exists($table, $database = ''){
		$list = mysql_query("SHOW TABLES FROM ".$database."", $this->link);
		for ($i = 0; $i < mysql_num_rows($list); $i++){
			if (mysql_query("SHOW TABLES FROM ".$list."", $i) == $this->prefix.$table){
				return true;
			}
		}
		...
		foreach ($arg['values'] as $k => $v){
			if ($k != 'primary'){
				if ($k == 'type' or ($k == 'name' and $arg['action'])){
					...
					$result[] = $k.$separator_in.'\''.mysql_real_escape_string($v).'\'';
				}
			}
		}
