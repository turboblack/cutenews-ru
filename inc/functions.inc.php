...
global $DOCUMENT_ROOT;
	...
	if (is_file($url['abs'])){
        $url_file_arr = explode('/', $url['path']);
        $url['file'] = end($url_file_arr);
...
global $PHP_SELF, $QUERY_STRING, $allow_full_story, $xfields, $config_http_home_url, $config_http_script_dir, $config_rufus, $user_id;
    ...
    $cat_arr = explode(',', $arr['category']);
    $cat = reset($cat_arr);
    $category_arr = explode('/', category_get_link($cat));
    $link_arr = run_filters('cute-get-link', array('arr' => $arr, 'link' => $for[$type]));
	$link	= end($link_arr);
	...
	$link	= strtr($link, array(
              ...
              '{category}'	  => ($cat ? end($category_arr) : 'none'),
