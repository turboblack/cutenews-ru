	...
	foreach ($ListOfDatabasesToMaybeBackUp as $dbname) {
		...
		if (is_resource($tables)) {
			...
			while (list($tablename) = mysql_fetch_array($tables)) {
				$TableStatusResult = mysql_query('SHOW TABLE STATUS LIKE "'.mysql_real_escape_string($tablename).'"');
	...
	if (($GZ_enabled && ($zp = @gzopen($backupabsolutepath.$tempbackupfilename, 'wb'))) ||
		...
		foreach ($SelectedTables as $dbname => $value) {
			...
			for ($t = 0; $t < count($SelectedTables[$dbname]); $t++) {
				...
				$TableStatusResult = mysql_query('SHOW TABLE STATUS LIKE "'.mysql_real_escape_string($SelectedTables[$dbname][$t]).'"');
				if (!($TableStatusRow = mysql_fetch_array($TableStatusResult))) {
								...
								switch ($RowTypes[$dbname][$SelectedTables[$dbname][$t]][$val]) {
									...
									case 'longblob':
										if ($HexBLOBs) {
											...
											$valuevalues[] = QUOTECHAR.mysql_real_escape_string($row[$key]).QUOTECHAR;
										}
										break;
									...
									case 'decimal':
									case 'year':
										$valuevalues[] = mysql_real_escape_string($row[$key]);
										break;
